...
var anatomogramModule = (function ($) {
	...
	function togglePathColor(path, evtType, name, select, sex) {
		...
		if (evtType === undefined) {
			...
			var head = "TissueAnalyzer";
			url = url.substring(0,url.lastIndexOf(head)+head.length);
			url = url + "?select="+name+"&button=submit&sex="+sex+"";
			window.location.href = url;
			/*window.location.href =
				"http://pvjs.wikipathways.org/index.php/Special:TissueAnalyzer?select="+name+"&button=submit&sex="+sex+"";*/
	...
	function init(allSvgPathIds, fileNameMale, fileNameFemale, select, sex) {
		...
		if (fileNameMale !== fileNameFemale) {
			...
			$("#sex-toggle-image").button().toggle(
					function () {
						$(this).attr("src","/wpi/extensions/TissueAnalyzer/images/female_selected.png");
						loadAnatomogram(fileNameFemale, allSvgPathIds, select, "female");
					},
					function () {
						$(this).attr("src","/wpi/extensions/TissueAnalyzer/images/male_selected.png");
