 ...
 *
 * This page was adapted from the MediaWiki special page
 * 'SpecialListusers.php'. Original credits above.
 *
 * Editor: Bernhard Hoisl <berni@hoisl.com>
 */
 ...
 * next to their names.
 */
class SpecialSocialRewardingListUsers extends QueryPage {
	/* public  */ var $requestedGroup = '';
	/* public  */ var $requestedUser = '';

	function getName() {
		return 'SocialRewardingListUsers';
	}
	...
	function getPageHeader( ) {
		...
		$title = Title::makeTitle( NS_SPECIAL, 'SocialRewardingListUsers' );
	...
	function formatResult( $skin, $result ) {
		...
		// Load SocialRewarding classes and display stars
		$reward = $this->loadReward();
		$name .= " " . $reward->getDisplayStars($user);
	...
	/**
	 * Load SocialRewarding classes and data.
	 *
	 * @access private
 ...
 */
function wfSpecialSocialRewardingListUsers( $par = null ) {
	global $wgRequest, $wgContLang;

	list( $limit, $offset ) = wfCheckLimits();


	$slu = new SpecialSocialRewardingListUsers();
